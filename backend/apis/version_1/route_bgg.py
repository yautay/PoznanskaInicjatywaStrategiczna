...
def retrieve_collection(data: CollectionCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user_from_token)):
    if data.bgg_user == retrieve_users_by_id(current_user.id, db=db).bgg_user or current_user.superuser or current_user.administrator:
